...
# CheatSlips gates cheat content behind a login (with reCAPTCHA on the form,
# so we don't automate the login itself). Instead we reuse a session cookie
# obtained by logging in manually in a browser: copy the "Cookie" request
# header from devtools after logging in and store it as the
# CHEATSLIPS_SESSION_COOKIE secret/env var. It needs to be refreshed
# manually whenever the session expires.
CHEATSLIPS_SESSION_COOKIE = os.getenv("CHEATSLIPS_SESSION_COOKIE")
if CHEATSLIPS_SESSION_COOKIE:
    SCRAPER.headers["Cookie"] = CHEATSLIPS_SESSION_COOKIE


def cheatslips_session_is_valid() -> bool:
    """Sanity check that CHEATSLIPS_SESSION_COOKIE is actually logged in."""
    if not CHEATSLIPS_SESSION_COOKIE:
        return False
    response, err = fetch_with_retry("https://www.cheatslips.com/account", SCRAPER, max_retries=1)
    if not response:
        return False
    return "/login" not in response.url and 'href="/login"' not in response.text
...
def process_cheatslips(title_names: dict[str, str], title_ids: list[str], verbose: bool = False):
    if not CHEATSLIPS_SESSION_COOKIE:
        print("  CheatSlips: CHEATSLIPS_SESSION_COOKIE not set, skipping (cheat content requires login)")
        return
    if not cheatslips_session_is_valid():
        print("  CheatSlips: session cookie looks expired/invalid, skipping. Log in again in a browser and refresh the CHEATSLIPS_SESSION_COOKIE secret.")
        return
